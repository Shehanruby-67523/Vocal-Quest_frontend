...
function DemonGuardian() {
  const handleCommand = (command) => {
    console.log(`Voice command selected: ${command}`)
  }
  ...
  return (
    <div className="min-h-screen bg-navy-950 text-slate-100">
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
        <section className="space-y-8 rounded-2xl border border-slate-300/10 bg-navy-900/50 p-4 shadow-panel sm:p-6">
          <article className="relative overflow-hidden rounded-2xl border border-gold-500/20">
            <div className="absolute right-4 top-4 z-20 rounded-full border border-gold-400/40 bg-navy-950/85 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-gold-300">
              ... Listening
            </div>
            <div className="relative h-52 bg-gradient-to-b from-navy-800 via-navy-850 to-navy-950 sm:h-72">
          ...
          <article className="rounded-xl border border-slate-300/10 bg-navy-900/80 p-4 sm:p-6">
          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <button
                type="button"
                aria-label="Approach"
                aria-describedby="approach-desc"
                onClick={() => handleCommand('APPROACH')}
                className="rounded-xl border border-gold-400/35 bg-navy-850 p-5 text-left transition hover:border-gold-300 hover:bg-navy-800"
              >
                <p className="text-3xl font-bold tracking-wide text-gold-300">APPROACH</p>
                <p id="approach-desc" className="mt-2 text-lg text-slate-300/90">
                  "Walk forward with intent..."
                </p>
              </button>
              <button
                type="button"
                aria-label="Sneak"
                aria-describedby="sneak-desc"
                onClick={() => handleCommand('SNEAK')}
                className="rounded-xl border border-gold-400/35 bg-navy-850 p-5 text-left transition hover:border-gold-300 hover:bg-navy-800"
              >
                <p className="text-3xl font-bold tracking-wide text-gold-300">SNEAK</p>
                <p id="sneak-desc" className="mt-2 text-lg text-slate-300/90">
                  "Move quietly through shadows..."
                </p>
              </button>
            </div>
          </section>
